package com.drummingfish.extrastuff.item;

public class ItemInfo {

	// Name of folder inside assets
	public static final String TEXTURE_LOCATION = "extrastuff";
	
	public static int CHAINMAIL_ID;
	public static final String CHAINMAIL_KEY = "chainmail";
	public static final int CHAINMAIL_DEFAULT = 25000;
	public static final String CHAINMAIL_UNLOCALIZED_NAME = "chainmail";
	public static final String CHAINMAIL_NAME = "Chainmail";
	public static final String CHAINMAIL_ICON = "chainmail";
	
	public static int IRON_NUGGET_ID;
	public static final String IRON_NUGGET_KEY = "ironNugget";
	public static final int IRON_NUGGET_DEFAULT = 25001;
	public static final String IRON_NUGGET_UNLOCALIZED_NAME = "ironNugget";
	public static final String IRON_NUGGET_NAME = "Iron Nugget";
	public static final String IRON_NUGGET_ICON = "nuggetIron";
	
	public static int EXTRABED_ID;
	public static final String EXTRABED_KEY = "extraBed";
	public static final int EXTRABED_DEFAULT = 25002;
	public static final String EXTRABED_UNLOCALIZED_NAME = "extraBed";
	public static final String EXTRABED_NAME = "Bed";
	public static final String EXTRABED_ICON = "bed";
	
	public static int DOOR_GOLD_ID;
	public static final String DOOR_GOLD_KEY = "goldDoor";
	public static final int DOOR_GOLD_DEFAULT = 25003;
	public static final String DOOR_GOLD_UNLOCALIZED_NAME = "goldDoor";
	public static final String DOOR_GOLD_NAME = "Gold Door";
	public static final String DOOR_GOLD_ICON = "goldDoor";
	
	public static int DOOR_DIAMOND_ID;
	public static final String DOOR_DIAMOND_KEY = "diamondDoor";
	public static final int DOOR_DIAMOND_DEFAULT = 25004;
	public static final String DOOR_DIAMOND_UNLOCALIZED_NAME = "diamondDoor";
	public static final String DOOR_DIAMOND_NAME = "Diamond Door";
	public static final String DOOR_DIAMOND_ICON = "diamondDoor";
	
	public static int DOOR_EMERALD_ID;
	public static final String DOOR_EMERALD_KEY = "emeraldDoor";
	public static final int DOOR_EMERALD_DEFAULT = 25005;
	public static final String DOOR_EMERALD_UNLOCALIZED_NAME = "emeraldDoor";
	public static final String DOOR_EMERALD_NAME = "Emerald Door";
	public static final String DOOR_EMERALD_ICON = "emeraldDoor";
}
